...
def is_action_synonym($word; $canonical):
  ...
  if $i == null then false
    ...
    # Try both lowercase and original-case key
    ($i.action_synonyms[$canonical] // $i.action_synonyms[($canonical | ascii_upcase)] // []) |
    map(ascii_downcase) |
...
def is_object_synonym($word; $canonical):
  ...
  if $i == null then false
    ...
    # Try both lowercase and original-case key (handles "API", "PR")
    ($i.object_synonyms[$canonical] // $i.object_synonyms[($canonical | ascii_upcase)] // []) |
    map(ascii_downcase) |
    (morph_map[$word] // $word) as $norm_word |
    any(. as $syn |
...
# P1 negative: "something is great" → no match (not a diagnostic)
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
...
# P3 negative: "make it purple" → no match (not in adjective map)
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
...
# P3 multi-word: "make it a lot faster" → optimize (right-to-left scan)
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
...
# P5/P1 priority: "what is wrong" → caught by P1 as debug, not P5 as explain
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
...
# P6 negative: "discuss the plan I made" → no match (discuss not a recognized action)
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
...
# P7 minimum length: "can you help" → no match (remainder too short)
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
...
def preprocess_prompt:
  ...
  then
    ...
    if $found_verb != null and ($has_first_person or $has_imperative) then
      ...
      # Use morph_map to normalize plurals: "tests" → "test" matches action "test"
  ...
  then
    ...
    # Resolve synonym to canonical action: "check" → "review"
...
- [ ] **Step 2: Write test for compound intent detection**
...
# "fix the bug and add tests" should detect compound intent
result=$(jq -f "${PLUGIN_ROOT}/scripts/match.jq" \
  ...
  --slurpfile intents "${PLUGIN_ROOT}/data/intents.json" \
  "${PLUGIN_ROOT}/data/index.json" 2>/dev/null)
has_compound=$(printf '%s' "$result" | jq -r '.compound_intent.detected // false')
# May or may not detect depending on whether both candidates score > 0.50
